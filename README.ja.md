...
- **OAuth 2.1 クライアント** — 認可コードフロー（PKCE）、動的クライアント登録、トークンリフレッシュ、安全なトークン永続化を内蔵。MCP 認可仕様の関連 RFC に完全対応：
  ...
  - [RFC 8414](https://www.rfc-editor.org/rfc/rfc8414) Authorization Server Metadata の検出（§3 パス付き issuer のパス挿入ルール対応）
  ...
  - [RFC 7591](https://www.rfc-editor.org/rfc/rfc7591) Dynamic Client Registration（§3.2.1 `client_secret_expires_at` — 期限切れ時に自動再登録）
