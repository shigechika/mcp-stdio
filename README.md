...
- **MCP 2026-07-28 support** — both faces speak both protocol eras, and your own MCP client or server does not have to change:
  - **as a client** (`--protocol-era auto|modern|legacy`, default `legacy`): an `auto` run probes the remote with `server/discover` and, on a modern server, drops the `initialize` handshake and session id, adds the per-request `_meta` and `Mcp-Method`/`Mcp-Name` headers, keeps `subscriptions/listen` streams open for notifications, bridges MRTR back into the `elicitation/create` / `sampling/createMessage` / `roots/list` a 2025-era client understands, and makes a cancel actually abort the in-flight request (client relay since v0.31.0)
  - **as a server** (`mcp-stdio serve`): a modern client gets `server/discover` and stateless dispatch with `resultType` / `ttlMs` / `cacheScope` stamping (`--cache-ttl-ms`), while legacy clients keep sessions exactly as before — one endpoint, both eras, your stdio server untouched
  - verified end to end against **python-sdk v2.0.0** in both directions: its client drives our `serve`, and our relay drives its server
  - → details in **[Protocol eras](https://shigechika.github.io/mcp-stdio/modes/#protocol-eras)**
