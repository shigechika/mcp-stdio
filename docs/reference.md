...
| `--max-message-size BYTES` | 10 MiB | Cap on a single upstream response body (JSON or cumulative SSE stream) buffered before parsing; 0 disables the cap. Also sends `Accept-Encoding: identity` by default (#416, #417); `-H 'Accept-Encoding: gzip'`/`deflate` opts back in and decodes through a genuinely size-bounded decompressor (#418) — any other negotiated coding (stacked, or one this relay has no bounded decoder for) needs `--max-message-size 0` to be accepted at all. Also applies to this relay's own OAuth HTTP traffic (discovery, DCR, token exchange/refresh, device-flow polling) — an authorization server is exactly as untrusted a peer as the MCP server itself (#419) |
...
- Claude Desktop's stdio spawn behavior (Cowork/Code sibling probe)
- mcp-remote (TypeScript MCP client)
- MCP SDKs (TypeScript & Python)
- Windows stdio handling

mcp-stdio works around these issues at the wire level, or via `serve` as an alternative deployment, where possible.
