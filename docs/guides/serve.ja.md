...
- セッション処理は厳密かつライフサイクル準拠です：リクエストは
  ...
  よって相関されるため、id を*逐次*再利用する（同時に未処理のリクエストは
  1 本だけ）クライアントは正しく対応付けられます。一方、同一セッション上で
  *同じ* id を持つリクエストを 2 本*同時に*未処理にすると、原理的に区別
  できません（子プロセスからの 2 つの応答が同じ id を持つため）——serve は
  pending を last-writer-wins で解決するので、一方の呼び出しがもう一方の
  応答を受け取り得ます。JSON-RPC id は未処理リクエストごとに一意に保って
  ください。なお、毎回新しいセッション id を発行するクライアントに状態を
  保持させることはできません——それはクライアント自身の挙動です。
