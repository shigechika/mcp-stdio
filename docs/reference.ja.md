...
新しいクライアントは、ツール・プロンプト・リソースの一覧が変わったことを
受け取るための長時間接続も開けます（→
[一覧が変わったことをクライアントに伝える](modes.md#listchanged-serve)）。
そのためのフラグはありません。同時に開ける本数は認証ユーザーごとに
**4 本**までで、5 本目は待たせずにその場で断ります。各接続は 15 秒ごとに
コメントを送るのでプロキシに切られにくく、接続が付いているバックエンドが
`--modern-idle-ttl` で回収されることはありません。
...
- Streamable HTTP transport、仕様リビジョン **2026-07-28** —— ケーパビリティの探索、リクエストごとのメタデータとヘッダ、セッション不要のリクエスト、一覧結果のキャッシュ指示、通知用の長時間接続（クライアント側と、一覧変更通知については `serve` 側も）、処理中のクライアントへの問い合わせ。クライアント側は `--protocol-era` でオプトイン。`serve` はひとつのエンドポイントで両方のリビジョンに応答する。python-sdk v2.0.0 に対して双方向で相互運用性を検証済み
