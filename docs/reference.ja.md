...
| `MCP_STDIO_MRTR_REVERSE_ENABLE` | `1` を設定すると、`serve` で公開した古いサーバーが呼び出しの途中で新しいクライアントに何かを尋ねられるようになる——入力の要求、sampling の依頼、roots の一覧取得——を MRTR 経由で行う、上記 `MCP_STDIO_MRTR_STRIP` の逆方向。既定は無効。OAuth 認証済みの呼び出し元にしかブリッジしない。詳細は[サーバーからの mid-call な問い合わせに答える](modes.md#answering-your-servers-mid-call-questions)を参照 |
