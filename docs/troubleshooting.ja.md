...
**解決方法：**
1. mcp-stdio をフォアグラウンドで実行し stderr を確認してください。認可サーバーが要求と異なるスコープを付与した場合、mcp-stdio は `authorization server granted scope: …` をログ出力します（RFC 6749 §5.1 上、トークンレスポンスに `scope` が入るのは要求と異なるときだけです）。この行が出なければ、要求どおりのスコープが付与されています。念のため認可サーバー自身のログや管理コンソールでも確認してください。
