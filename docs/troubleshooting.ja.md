...
**解決方法：**
1. mcp-stdio をフォアグラウンドで実行し stderr を確認してください。認可サーバーが要求と異なるスコープを付与した場合、mcp-stdio は `authorization server granted scope: …` をログ出力します（RFC 6749 §5.1 上、トークンレスポンスに `scope` が入るのは要求と異なるときだけです）。この行が出なければ、要求どおりのスコープが付与された可能性が高いです。ただし RFC 6749 §5.1 に準拠せずダウングレード時でも `scope` を省略するサーバーもあるため、スコープを原因から外す前に認可サーバー自身のログや管理コンソールで確認してください。
...
### modern era のサーバーから `400 Bad Request` と JSON-RPC `-32020` が返る

`-32020` は `HeaderMismatch` です：リクエストのヘッダと body が食い違って
おり、2026-07-28 のサーバーはどちらが正しいかを推測せず拒否しなければ
なりません。`mcp-stdio serve` は——そしてリモートの modern サーバーも——
次の場合にこれを返します。

- `MCP-Protocol-Version` ヘッダが無い、または一致しない（body の `_meta`
  にある `io.modelcontextprotocol/protocolVersion` と等しい必要があり、
  ヘッダ欠落も不一致として扱われる）；
- `Mcp-Method` が無い、または body の `method` と違う；
- `Mcp-Name` が body の `params.name`（`resources/read` なら `params.uri`）
  と、Base64 センチネルのデコード後に一致しない——壊れたセンチネルは
  何にもデコードされないので、これもここに落ちる；
- 同じルーティングヘッダを 2 回以上送っている。

mcp-stdio を**クライアントとして**使っている場合、これらはすべて
mcp-stdio 自身が組み立てます。したがってリモートからの `-32020` は、
設定ミスではなく途中の中継（プロキシやゲートウェイ）がヘッダを
書き換えていることを示唆します。

紛らわしい隣接コードがふたつあります：`-32602` は使える `_meta` が
そもそも無かったこと、`-32022` はプロトコルバージョンの形式は正しいが
サポート外であることを意味します。後者は `error.data.supported` に
サーバーが受け付けるバージョンが列挙されます。
