...
### サーバーが `400` とエラーコード `-32020` で拒否する

**これが意味すること：** リクエストの内容が食い違っている——ヘッダと本文が
別のことを言っている——とサーバーが判断し、推測せずに拒否しています。

**mcp-stdio でリモートサーバーに接続している場合**、原因はあなたの設定では
ありません。これらのヘッダは mcp-stdio 自身が組み立てて整合させています。
あいだにいる何か——たいていはプロキシや API ゲートウェイ——が書き換えて
います。それを経由せずに接続してみると切り分けられます。

**`mcp-stdio serve` を運用していて**クライアント側にこれが出る場合は、
そのクライアントが食い違ったリクエストを送っています。どのヘッダが
一致しなかったかはエラーメッセージに出ます。

似たエラーの見分け方：

| コード | 意味 | 対処 |
|---|---|---|
| `-32020` | ヘッダと本文が食い違っている | あいだでヘッダを書き換えているものを探す |
| `-32602` | 新しい仕様が必要とする情報が足りない | たいていはクライアント側の実装不足 |
| `-32022` | そのプロトコルバージョンに未対応 | 対応バージョンがエラーに列挙されるので、その中から選ぶ |
